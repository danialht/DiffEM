...
def evaluate(
    ...
    corrupted_dataset_name = f"{corruption_name}{corruption_level}" + ('_test' if test else '')
    ...
    # Load datasets
    corrupted_dataset = corrupted_dataset.remove_columns(['A'])
    ...
    original_dataset = original_dataset.remove_columns(['A'])
    original_dataset.set_format('numpy')

    original_dataset = original_dataset.select(range(min(len(original_dataset), 50_000)))
    corrupted_dataset = corrupted_dataset.select(range(min(len(corrupted_dataset), 50_000)))
    ...
    real_imgs = np.stack([np.array(img) for img in original_dataset['y']]).astype(np.uint8)
